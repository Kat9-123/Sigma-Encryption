...
public class Main : MonoBehaviour {
    ...
    private void Start()
    {
        //CreateKey(); //Go to key create function
        Encrypt(); //Go to encrypt function
    }
        ...
        for (int z = 0; z < InpL.Count; z++) //For each letter in the input list
        {
            ...
            for (int h = 1; h < Key.Length - 2; h++) //For the amount of segments in the key
            {
                for (int g = 0; g < 26; g++) //For each letter in said segment
                {
                    ...
                    if (CurrentEnc == AlphaList[g]) //If the that is being processed currently is equal to a letter in the alphabet. g=the index of said letter
                    {
